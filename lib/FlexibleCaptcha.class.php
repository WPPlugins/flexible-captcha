...
class FlexibleCaptcha {
	...
	function check_captcha_val() {
		...
		isset($_COOKIE['FC_captcha_key'][$_REQUEST['FC_captcha_unique_id']]) && preg_match('/^[a-f0-9]{32}$/', $_COOKIE['FC_captcha_key'][$_REQUEST['FC_captcha_unique_id']])) {
			...
			setcookie('FC_captcha_key['.$_REQUEST['FC_captcha_unique_id'].']', 0, time()-999999);
		}
		return $returnVal;
	}
	...
	function submit_background_settings() {
		if (wp_verify_nonce( $_POST['FC_nonce'], plugin_basename(__FILE__) )) {
			...
			print "Background settings successfully saved.";
		}
		die();
	}
	...
	function submit_font_settings() {
		if (wp_verify_nonce( $_POST['FC_nonce'], plugin_basename(__FILE__) )) {
			...
			print "Font settings successfully saved.";
		}
		die();
	}
	...
	function submit_gen_settings() {
		if (wp_verify_nonce( $_POST['FC_nonce'], plugin_basename(__FILE__) )) {
			...
			print "General settings successfully saved.";
		}
		die();
	}
	...
	function check_login_submit($user, $username, $password) {
		if ((isset($_POST['user-submit']) || (isset($_POST['wp-submit']) && $_POST['wp-submit'] == 'Log In')) && get_option('FC_add_to_login') == 1 && !$this->check_captcha_val()) {
